...
//             入库成功并跳转页面
                $this->success('添加成功',url('admin/index/lobby'));
            }else{
                $this->error('添加失败');
            }
...
//        $list=\think\Db::table('article')->with('category')->order('create_time DESC')->paginate(2);
...
//    删除
    public function delete()
    {
        ...
        $a=\app\index\model\article::where('id',$id)->delete();
        if(empty($a)) {
            return $this->error('失败');
        }else{
...
//            如果验证信息不正确则输出错误提示
            ...
            $aa =\app\index\model\article::where('id',$id)->update($data);
            if ($aa) {
                $this->success('修改成功');
            } else {
                $this->error('修改失败');
...
//        获取get请求
        if ($re->isGet()) {
            ...
            $this->assign('all',$all);
            //            把对象转换成数组  toArray
            $b= \app\index\model\article::where('id',$id)->find()->toArray();
            $this->assign('b', $b);
            return $this->fetch();
        }
    }
